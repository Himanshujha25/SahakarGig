# SahakarGig — Deep App Audit & SIH 2026 Real-World Readiness Report
...
|---|---|---|---|---|
| **1** | Provider Registration & Verification | `authController.js` (L53-59), `adminController.js` (L100-107) | ✅ **WORKING** | Unverified providers appear in search with active "Book Now" buttons (breaks at booking attempt instead of disabling in UI). |
| **2** | Worker Skill Profiling & Certification | `Provider.js`, `providerController.js` (`uploadDoc`), `Profile.jsx` | ✅ **WORKING** | Multer disk storage persists files properly to `/uploads`. Cloudinary/S3 fallback not configured for distributed multi-instance deployment. |
| **3** | Customer Booking & Scheduling | `bookingController.js`, `BookingRequest.jsx`, `JobDetail.jsx` | ✅ **WORKING** | No past-date validation for `scheduledTime` in `createBooking`. |
| **4** | Geo-location Based Service Matching | `helpers.js` (`haversine`), `providerController.js` (L23-26) | ⚠️ **PARTIAL (UI GAP)** | Backend Haversine is functional, but `FindServices.jsx` **never transmits user `lat`/`lng` query params**, rendering geo-filtering inactive in the frontend. |
| **5** | Digital Payments & Invoicing | `paymentController.js`, `Payment.jsx`, `Invoice.jsx` | ⚠️ **PARTIAL (CONFIG)** | Razorpay order creation + HMAC-SHA256 signature verification code is complete, but `RAZORPAY_KEY_ID` & `SECRET` in `server/.env` are empty strings. |
| **6** | Rating & Feedback System | `reviewController.js`, `Review.js` | ✅ **WORKING** | Guarded against duplicates with unique index `{ bookingId: 1, createdBy: 1 }`. Live recalculation and caching of Trust Score. |
| **7** | Worker Welfare & Insurance Integration | `welfareController.js`, `Welfare.jsx`, `Welfare.js` | ✅ **WORKING** | Welfare score calculated; QR generated via `qrcode`. `schemesEligible` is not auto-computed from profile attributes. |
| **8** | Emergency & On-Demand Booking | `bookingController.js` (L21-22), `BookingRequest.jsx`, `JobQueue.jsx` | ✅ **WORKING** | `isEmergency` flag triggers priority queuing and distinct UI badge. |
| **9** | Cooperative Federation Admin Dashboard | `federationController.js`, `Federation/Dashboard.jsx` | ✅ **WORKING** | Multi-tier aggregation across affiliated societies with financial ledger queries. |
| **10** | Multilingual Mobile Application | `en.json`, `hi.json`, `i18n.js`, `LangToggle.jsx` | ⚠️ **PARTIAL** | Core i18n setup exists, but several deep views (`FindServices`, `Earnings`, `Welfare`) have hardcoded English strings. |
| **11** | AI Demand Forecasting & Allocation | `aiController.js` (`demandForecast`, `nudgeProviders`) | ✅ **WORKING** | Aggregates demand by service and hour; pushes real-time `ai:nudge` socket events to idle workers. |
| **12** | Mobile PWA (Progressive Web App) | `public/manifest.json`, icon assets | ⚠️ **PARTIAL** | Manifest exists, but no Service Worker is registered in `index.html` or `main.jsx` for offline capability and native install prompts. |

---

## 3. Deep Bug & Vulnerability Breakdown

### 🔴 Critical Showstoppers (Must Fix for Live Demo & Real World)
...
### 🟠 High / Important Issues (Real-World & Demo Polish)
...
3. **"Is the architecture scalable across states and districts?"**
   - **Answer:** We employ a 3-tier hierarchy: **Federation** (State level) → **Cooperatives** (District/Taluka level) → **Providers** (Local level). Federations set overarching policies and track macro statistics; Cooperatives manage local operations, disputes, and member verification.
...
P0 (Demo Blockers - 15 mins)
 ├── Fill test keys in server/.env (RAZORPAY_KEY_ID, RAZORPAY_KEY_SECRET)
 ├── Fix FindServices.jsx API parsing: setProviders(data?.providers || [])
 ├── Add geolocation coordinate passing in FindServices.jsx
 └── Update seed.js to seed a default Federation & Federation Admin

P1 (UX Integrity - 30 mins)
 ├── Fix notify() call in paymentController.js (use provider.userId)
 ├── Populate chat.sender name in getBooking
 ├── Correct JobDetail.jsx chat sender field (m.sender instead of m.from)
 └── Register Service Worker in client/src/main.jsx for installable PWA

P2 (Security & Polish - 30 mins)
 ├── Add cooperativeId validation in adminController.resolveDispute
 ├── Whitelist fields in welfareController.upsertWelfare
 ├── Update Earnings.jsx to reflect net providerPayout
 └── Remove unused @prisma/client from server/package.json
```
