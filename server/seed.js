...
const { invoiceNumber } = require('./src/utils/helpers');
...
async function main() {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const coop = await Cooperative.create({
      name: `Labour Cooperative ${i + 1}`, registrationId: `REG-100${i}`, region: `Region ${i + 1}`, adminId: admin._id, commissionRate: 8,
    });
    coops.push(coop);
  }
  ...
  for (let i = 0; i < 15; i++) {
    ...
    const p = await Provider.create({
      ...
      verified: i > 2, availabilitySlots: [{ day: 'Mon', from: '09:00', to: '17:00' }],
    });
  ...
  for (const p of providers) {
    for (let k = 0; k < 3; k++) {
      ...
      if (status === 'completed') {
        await Review.create({ bookingId: b._id, rating: 4 + Math.floor(Math.random() * 2), comment: 'Good service', createdBy: h._id });
        ...
        const pay = await Payment.create({ bookingId: b._id, amount: price, cooperativeCommission: commission, providerPayout: payout, status: 'released' });
        ...
        await Welfare.findOneAndUpdate({ providerId: p._id }, { $inc: { totalEarnings: payout, daysWorked: 1 } }, { upsert: true });
      }
    }
  }
  ...
  await Booking.create({ householdId: dh._id, providerId: dp._id, cooperativeId: dp.cooperativeId, service: dp.skills[0], status: 'disputed', price: dp.hourlyRate * 2 });

  console.log('[seed] complete: 3 coops, 15 providers, 10 households, bookings, reviews, welfare.');
  process.exit(0);
}
